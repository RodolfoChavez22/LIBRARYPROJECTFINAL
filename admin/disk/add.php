        ...
        if(isset($_REQUEST['button-submit'])){
            ...
            if(empty($errorMsg)){
                ...
                if(empty($errorMsg)){
                    ...
                    $sql = "INSERT INTO disk (Name,Publisher,Publish_date,Rental_Status,Img,Description) VALUES (?,?,?,?,?,?)";
                    $stmt = mysqli_stmt_init($con);
                    if(!mysqli_stmt_prepare($stmt,$sql)){echo "SQL statement failed";}
                    else{
                        mysqli_stmt_bind_param($stmt, "sssiss", $Name, $Publisher, $Date, $Rental, $fileNameNew, $Description);
                        mysqli_stmt_execute($stmt);
                    }
                    //Retrieve the disk id of from the previous insert
                    $InsertId = mysqli_insert_id($con);
                    //Set Multiple Authors to retrieved disk id
                    $sql = "INSERT INTO authordiskjoin (Author_id,Disk_id) VALUES (?,?)";
                    $stmt = mysqli_stmt_init($con);
                    if(!mysqli_stmt_prepare($stmt,$sql)){echo "SQL statement failed";}
                    else{
                    ...
                    //Set Multiple Genres to retrieved disk id
                    $sql = "INSERT INTO genrediskjoin (Genre_id,Disk_id) VALUES (?,?)";
                    $stmt = mysqli_stmt_init($con);
                    if(!mysqli_stmt_prepare($stmt,$sql)){echo "SQL statement failed";}
                    else{
...
<div class="page-content">
    <div class = "display-body">
        <form  action = "" method="POST" class="add-form" id="add-form" enctype="multipart/form-data">
            <h3 class="add-header">Create Disk</h3>
            <div class = "add-input">
                ...
                <div class= "add-genre">
                    <div class="add-margin">
                        ...
                        <div class = "add-form-types">
                            ...
                            <input type="text" placeholder="Disk name..." class="input-single" name="name" value="<?php echo isset($_POST['name']) ? $_POST['name'] : '' ?>"><br>
                        ...
                        <div class = "signup-button">
                            <button type="submit" name ="button-submit" style = "margin-left: 5%;">CREATE DISK</button>
                        </div>
                    </div>
                </div>
            </div>
