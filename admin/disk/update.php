<?php
    ...
    $id=$_GET['diskid'];
    $sql="SELECT disk.Disk_id, disk.Name, disk.Rental_status, disk.Publisher, disk.Publish_date, disk.Deleted_id, disk.Img, disk.Description,
    ...
    //Selects Authors
    $sql="SELECT * FROM authordiskjoin WHERE Disk_id=$id";
    $result=mysqli_query($con,$sql);
    $x = 1;
    ...
    $_SESSION['Amount'] = $x - 1;
    //Select Genres
    $sql="SELECT * FROM genrediskjoin WHERE Disk_id=$id";
    $result=mysqli_query($con,$sql);
    $x = 1;
    while($row=mysqli_fetch_assoc($result)){
        ...
        if(isset($_REQUEST['button-submit'])){
            ...
            if(empty($errorMsg)){
                ...
                if(empty($errorMsg)){
                    ...
                    //Update into disk table
                    if(!empty($fileNameNew)){
                        $Img = '../../img/' . $Img;
                        unlink($Img);
                        $Img = $fileNameNew;
                    }
                    $sql = "UPDATE disk SET Name = ?, Publisher = ?, Publish_date = ?, Rental_Status = ?, Img = ?, Description = ? WHERE Disk_id = $id";
                    $stmt = mysqli_stmt_init($con);
                    if(!mysqli_stmt_prepare($stmt,$sql)){echo "SQL statement failed";}
                    else{
                        mysqli_stmt_bind_param($stmt, "sssiss", $Name, $Publisher, $Date, $Rental, $Img, $Description);
                        mysqli_stmt_execute($stmt);
                    }
                    ...
                    $x = 0;
                    while($row=mysqli_fetch_assoc($result)){
                        $CurrentID = $row['DAuthor_Join_id'];
                        $sql2 = "UPDATE authordiskjoin SET Author_id = ? WHERE DAuthor_Join_id = $CurrentID";
                        $stmt = mysqli_stmt_init($con);
                    ...
                    if($_SESSION['Amount'] < $x){
                        ...
                        $sql = "DELETE FROM authordiskjoin WHERE Disk_id = $id ORDER BY DAuthor_Join_id DESC LIMIT $Difference";
                    ...
                    if($_SESSION['Amount'] > $x){
                        ...
                        for($c = 0; $c < $Difference; $c+=1){
                            $sql = "INSERT INTO authordiskjoin (Disk_id, Author_id) VALUES(?,?)";
                            $stmt = mysqli_stmt_init($con);
                            if(!mysqli_stmt_prepare($stmt,$sql)){echo "SQL statement failed";}
                            else{
                    ...
                    $x = 0;
                    while($row=mysqli_fetch_assoc($result)){
                        $CurrentID = $row['DGenre_Join_id'];
                        $sql2 = "UPDATE genrediskjoin SET Genre_id = ? WHERE DGenre_Join_id = $CurrentID";
                        $stmt = mysqli_stmt_init($con);
                        if(!mysqli_stmt_prepare($stmt,$sql2)){echo "SQL statement failed";}
                        else{
                    ...
                    if($_SESSION['GenreAmount'] < $x){
                        ...
                        $sql = "DELETE FROM genrediskjoin WHERE Disk_id = $id ORDER BY DGenre_Join_id DESC LIMIT $Difference";
                        mysqli_query($con,$sql);
                    }
                    ...
                    if($_SESSION['GenreAmount'] > $x){
                        ...
                        for($c = 0; $c < $Difference; $c+=1){
                            $sql = "INSERT INTO genrediskjoin (Disk_id, Genre_id) VALUES(?,?)";
                            $stmt = mysqli_stmt_init($con);
                            if(!mysqli_stmt_prepare($stmt,$sql)){echo "SQL statement failed";}
                            else{
                ...
                <div class= "add-genre">
                    <div class="add-margin">
                        ...
                        <div class = "add-form-types">
                            <label class="label-single">Name of Disk</label><br>
                            <?php if(isset($errorMsg[4])){echo "<p style='color:red; font-size:15px; margin-top: 5px; margin-left: 10px;'>".$errorMsg[4]."</p>";}?>
                            <input type="text" placeholder="Disk name..." class="input-single" name="name" value="<?php echo $Name;?>"><br>
                        ...
                        <div class = "signup-button">
                            <button type="submit" name ="button-submit" style = "margin-left: 5%;">UPDATE DISK</button>
                        </div>
                    </div>
                </div>
            </div>
