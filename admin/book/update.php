<?php
    ...
    $id=$_GET['bookid'];
    $sql="SELECT book.Book_id, book.Name, book.Rental_status, book.ISBN, book.Pages, book.Publisher, book.Publish_date, book.Deleted_id, book.Img, book.Description,
    ...
    //Selects Authors
    $sql="SELECT * FROM authorbookjoin WHERE Book_id=$id";
    $result=mysqli_query($con,$sql);
    $x = 1;
    ...
    $_SESSION['Amount'] = $x - 1;
    //Select Genres
    $sql="SELECT * FROM genrebookjoin WHERE Book_id=$id";
    $result=mysqli_query($con,$sql);
    $x = 1;
    while($row=mysqli_fetch_assoc($result)){
    ...
    if(isset($_POST["amount"]) || isset($_POST["genre-amount"]) || isset($_REQUEST['button-submit'])){
        ...
        if(isset($_REQUEST['button-submit'])){
            ...
            //ISBN must be 13 in length exactly
            if(empty($ISBN)){$errorMsg[5] = 'ISBN Required';}
            if(!empty($ISBN)) {
                if(strlen($ISBN) < 13 || strlen($ISBN) > 13){
                    $errorMsg[6] = 'ISBN Must Be 13 Numbers';
                }
                $sql = "SELECT * FROM book WHERE ISBN = $ISBN";
                $result = mysqli_query($con, $sql);
                //Check if the ISBN has already been registered
                if(mysqli_num_rows($result) > 0 && $TempISBN !== $ISBN){
                    $errorMsg[6] = 'ISBN already in use';
                }
            }
            ...
            if(empty($errorMsg)){
                ...
                if(empty($errorMsg)){
                    ...
                    $sql = "UPDATE book SET Name = ?, ISBN = ?, Pages = ?, Publisher = ?, Publish_date = ?, Rental_Status = ?, Img = ?, Description = ? WHERE Book_id = $id";
                    $stmt = mysqli_stmt_init($con);
                    if(!mysqli_stmt_prepare($stmt,$sql)){echo "SQL statement failed";}
                    else{
                        mysqli_stmt_bind_param($stmt, "ssississ", $Name, $ISBN, $Pages, $Publisher, $Date, $Rental, $Img, $Description);
                        mysqli_stmt_execute($stmt);
                    }
                    ...
                    $x = 0;
                    while($row=mysqli_fetch_assoc($result)){
                        $CurrentID = $row['Author_Join_id'];
                        $sql2 = "UPDATE authorbookjoin SET Author_id = ? WHERE Author_Join_id = $CurrentID";
                        $stmt = mysqli_stmt_init($con);
                    ...
                    if($_SESSION['Amount'] < $x){
                        ...
                        $sql = "DELETE FROM authorbookjoin WHERE Book_id = $id ORDER BY Author_Join_id DESC LIMIT $Difference";
                    ...
                    if($_SESSION['Amount'] > $x){
                        ...
                        for($c = 0; $c < $Difference; $c+=1){
                            $sql = "INSERT INTO authorbookjoin (Book_id, Author_id) VALUES(?,?)";
                            $stmt = mysqli_stmt_init($con);
                            if(!mysqli_stmt_prepare($stmt,$sql)){echo "SQL statement failed";}
                            else{
                    ...
                    $x = 0;
                    while($row=mysqli_fetch_assoc($result)){
                        $CurrentID = $row['Genre_Join_id'];
                        $sql2 = "UPDATE genrebookjoin SET Genre_id = ? WHERE Genre_Join_id = $CurrentID";
                        $stmt = mysqli_stmt_init($con);
                        if(!mysqli_stmt_prepare($stmt,$sql2)){echo "SQL statement failed";}
                        else{
                    ...
                    if($_SESSION['GenreAmount'] < $x){
                        ...
                        $sql = "DELETE FROM genrebookjoin WHERE Book_id = $id ORDER BY Genre_Join_id DESC LIMIT $Difference";
                        mysqli_query($con,$sql);
                    }
                    ...
                    if($_SESSION['GenreAmount'] > $x){
                        ...
                        for($c = 0; $c < $Difference; $c+=1){
                            $sql = "INSERT INTO genrebookjoin (Book_id, Genre_id) VALUES(?,?)";
                            $stmt = mysqli_stmt_init($con);
                            if(!mysqli_stmt_prepare($stmt,$sql)){echo "SQL statement failed";}
                            else{
...
<div class="page-content">
    <div class = "display-body">
        <form  action = "" method="POST" class="add-form" id="add-form" enctype="multipart/form-data">
            <h3 class="add-header">Update Book</h3>
            <div class = "add-input">
                ...
                <div class= "add-genre">
                    <div class="add-margin">
                        ...
                        <div class = "add-form-types">
                            <label class="label-single">Name of Book</label><br>
                            <?php if(isset($errorMsg[4])){echo "<p style='color:red; font-size:15px; margin-top: 5px; margin-left: 10px;'>".$errorMsg[4]."</p>";}?>
                            <input type="text" placeholder="Book name..." class="input-single" name="name" value="<?php echo $Name;?>"><br>
                            <label class="label-single">ISBN</label><br>
                            <?php 
                            ...
                            ?>
                            <input type="text" placeholder="ISBN..." class="input-single" name="isbn" value="<?php echo $ISBN; ?>"><br>
